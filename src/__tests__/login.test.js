...
describe('LoginSignUp component', () => {
  ...
  // Test the behavior of the popup cancel button
  it('handles popup cancel action', async () => {
    render(<LoginSignUp />);
    const input = screen.getByPlaceholderText('username');
    fireEvent.change(input, { target: { value: 'newuser' } });
    const button = screen.getByRole('button', { name: 'Login/Signup' });
    ...
    expect(screen.queryByText('The user "newuser" was not found. Do you want to create a new user?')).not.toBeInTheDocument();
  });
});
